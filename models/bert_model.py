...
def create_train_data(train_file,MAX_LEN,bs):
    ...
    tag_values=['O','PAD','total-participants', 'control-participants', 'intervention-participants', 'age', 'ethinicity',
                'location', 'eligibility', 'condition', 'intervention', 'control', 'outcome-measure', 'outcome',
                'iv-bin-abs', 'iv-bin-percent', 'iv-cont-mean', 'iv-cont-sd','iv-cont-median', 'iv-cont-q1', 'iv-cont-q3',
                'cv-bin-abs', 'cv-bin-percent', 'cv-cont-mean', 'cv-cont-sd', 'cv-cont-median', 'cv-cont-q1', 'cv-cont-q3',
    ...
    tags = pad_sequences([[tag2idx.get(l) for l in lab] for lab in labels],maxlen=MAX_LEN, dtype="long", value=tag2idx['PAD'], truncating="post", padding="post")
...
def model_training (epochs, learning_rate, batch_size,tr_file,te_file,MAX_LEN,mpath,saveFolder):
    ...
    hyperparameterFile=open(os.path.join(saveFolder,'biobert_paramsCheck.csv'),'a')
    ...
    model = BertForTokenClassification.from_pretrained(mpath,num_labels=len(tag2idx),output_attentions = False,output_hidden_states = False)
...
#     optimizer_grouped_parameters = [{"params": [p for n, p in param_optimizer]}]
    ...
    for ep in range(epochs):
        ...
        if val_F1>=best_F:
            ...
            report_df.to_csv(os.path.join(classR_directory,'biobert_best_classification_report.csv'))
                             
            
            for i in range(len(te_tokenized_text)):
                p, t, ppid= predictions[i], true_labels[i],pmidss[i]
                ...
                idss = [pi for pi in ppid if pi != 5]
                ...
                for pid, tk, tl, pl in zip(df_pid, df_tokens, df_true_labels, df_pred_labes):
                    tk = str(tk)
                    if tk.startswith('##'):
                        Ntoken[-1] = Ntoken[-1] + tk[2:]
                    else:
                        Ntoken.append(tk)
                        Npid.append(pid)
                        NtrueL.append(tl)
...
bertModeldirectory='../biobert_model_results'
bertModeldirectory=os.path.join(bertModeldirectory, str(seed_val))
try:os.mkdir(bertModeldirectory)
except:pass

bertModeldirectory=os.path.join(bertModeldirectory, bertmodelname)
try:os.mkdir(bertModeldirectory)
except:pass
...
for param_set in params_Set:
    ...
    try:
        model_training(epochs = param_set['epochs'], learning_rate = param_set['lr'],batch_size = param_set['batch'], MAX_LEN = 510, tr_file=tr_file,te_file=te_file, mpath=bertmodelpath,saveFolder=bertModeldirectory)
